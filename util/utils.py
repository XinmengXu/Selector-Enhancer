...
def overlap_cat(chunk_list, dim=-1):
    ...
    for i, chunk in enumerate(chunk_list):
        first_half, middle_one ,last_half = torch.split(chunk, chunk.size(-1) // 2, dim=dim)
        last_half = torch.cat([middle_one, last_half], dim=dim)
        if i == 0:
...
def sample_fixed_length_data_aligned(data_a, data_b, sample_length):
    ...
    if len(data_a) < sample_length:
        a = np.zeros(sample_length-len(data_a))
        data_a = np.concatenate((data_a, a))
        data_b = np.concatenate((data_b, a))
