...
class BaseTrainer:
    def __init__(self,
        ...
        self.model2.load_state_dict(load_checkpoint("C:/Users/a/Desktop/Wave-U-Net-for-Speech-Enhancement/train/checkpoints/latest_model.tar", self.device))
        if self.n_gpu > 1:
            self.model1 = torch.nn.DataParallel(self.model1, device_ids=list(range(self.n_gpu)))
            self.model2 = torch.nn.DataParallel(self.model2, device_ids=list(range(self.n_gpu)))
        # Trainer
        ...
        self._print_networks([self.model2])
    ...
    def _resume_checkpoint(self):
        ...
        if isinstance(self.model, torch.nn.DataParallel):
            self.model2.module.load_state_dict(checkpoint["model"])
        else:
    ...
    def _save_checkpoint(self, epoch, is_best=False):
        ...
        if isinstance(self.model2, torch.nn.DataParallel):  # Parallel
            state_dict["model"] = self.model2.module.cpu().state_dict()
        else:
            state_dict["model"] = self.model2.cpu().state_dict()
